...
class Publikasi_ilmiah extends CI_Controller
{
    ...
    public function index()
    {
        $data = [
            ...
            'user' => $this->Model_administrator_publikasi_ilmiah->findUser(['email' => $this->session->userdata('email')]),
            'publikasiIlmiahAll' => $this->Model_administrator_publikasi_ilmiah->getAllPublikasiIlmiah()
        ];

        $this->load->view('backend/templates/header', $data);
        $this->load->view('backend/templates/sidebar_administrator', $data);
        $this->load->view('backend/templates/topbar', $data);
        $this->load->view('backend/administrator/publikasi_ilmiah_index', $data);
    ...
    public function tambah_publikasi_ilmiah()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            redirect(base_url('administrator/publikasi_ilmiah'));
        } else {
            ...
            $level_penulis = $this->input->post('level_penulis');
            if ($level_penulis == '') {
            ...
            if ($this->upload->do_upload('file_publikasi_ilmiah')) {
                ...
                $this->Model_administrator_publikasi_ilmiah->tambahDataPublikasiIlmiah($dataTambah);
    ...
    public function detail_publikasi_ilmiah($id = false)
    {
        ...
        $data = [
            ...
            'uriSegment' => $this->uri->segment(2),
            'user' => $this->Model_administrator_publikasi_ilmiah->findUser(['email' => $this->session->userdata('email')]),
            'publikasiIlmiah' => $this->Model_administrator_publikasi_ilmiah->getPublikasiIlmiahData(['id' => $id])
        ];

        $this->load->view('backend/templates/header', $data);
        $this->load->view('backend/templates/sidebar_administrator', $data);
        $this->load->view('backend/templates/topbar', $data);
        $this->load->view('backend/administrator/publikasi_ilmiah_detail', $data);
        $this->load->view('backend/templates/footer', $data);
    }
    ...
    public function edit_publikasi_ilmiah()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($this->upload->do_upload('file_publikasi_ilmiah')) {
                ...
                $this->Model_administrator_publikasi_ilmiah->editDataPublikasiIlmiah(['id' => $id], $dataEdit);
    ...
    public function hapus_publikasi_ilmiah()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->Model_administrator_publikasi_ilmiah->hapusDataPublikasiIlmiah(['id' => $id]);
